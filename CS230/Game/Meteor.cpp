...
//Meteor::Meteor() :
//	GameObject({ (static_cast<double>(std::rand()) / RAND_MAX) * Engine::GetWindow().GetSize().x, (static_cast<double>(std::rand()) / RAND_MAX) * Engine::GetWindow().GetSize() .y})
//{
//    AddGOComponent(new CS230::Sprite("Assets/Meteor.spt", this));
//	SetVelocity({ (static_cast<double>(rand()) / RAND_MAX) * (2 * default_velocity) - default_velocity, (static_cast<double>(rand()) / RAND_MAX) * (2 * default_velocity) - default_velocity });
//	SetRotation((static_cast<double>(rand()) / RAND_MAX) * (2 * PI));
//	SetPosition({ (static_cast<double>(rand()) / RAND_MAX) * Engine::GetWindow().GetSize().x, (static_cast<double>(rand()) / RAND_MAX) * Engine::GetWindow().GetSize().y });
//    Engine::GetLogger().LogDebug("Rotation: " + std::to_string(GetRotation()));
//
//    //sprite.Load("Assets/Meteor.spt");
//    //AddGOComponent(new CS230::Sprite("Assets/Meteor.spt"));
//}
    ...
    CS230::GameObject({ 0, 0 })
{
    ...
    if (parent == nullptr) {
        ...
        //SetPosition({ ((double)rand() / RAND_MAX) * Engine::GetWindow().GetSize().x, ((double)rand() / RAND_MAX) * Engine::GetWindow().GetSize().y });
    }
    else {
